...
const FAKE_STATS: StatResponse = {
  totalChats: 1248,
  memberUserCount: 312,
  guestSessionCount: 936,
  avgResponseTime: 843,
  ...
  topQuestions: [
    { question: "입실 시간이 언제인가요?", count: 142 },
    { question: "외박 신청은 어떻게 하나요?", count: 118 },
    { question: "세탁실 이용 방법을 알려주세요", count: 97 },
    { question: "식당 운영 시간이 궁금해요", count: 85 },
    { question: "택배는 어디서 받나요?", count: 76 },
    { question: "냉난방 신청 방법이 궁금해요", count: 64 },
    { question: "퇴사 절차가 어떻게 되나요?", count: 58 },
    { question: "방 청소 일정이 어떻게 되나요?", count: 51 },
    { question: "wifi 비밀번호가 뭔가요?", count: 47 },
    { question: "공용 냉장고 이용 규칙이 있나요?", count: 43 },
  ],
};
...
function useStatistics(triggerAlert: (title: string, msg: string) => void) {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const sorted = [...FAKE_STATS.dailyStats].sort(
      (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
    );
    setStats({ ...FAKE_STATS, dailyStats: sorted });
    setLoading(false);
  }, [triggerAlert]);
