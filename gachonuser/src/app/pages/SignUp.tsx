...
import { User, Lock, Mail, Phone, ArrowLeft, KeyRound, CheckCircle, Building2, Home, ChevronDown, Check, ShieldCheck } from "lucide-react";
...
export default function SignUp() {
  ...
  const formRefs = {
    ...
    dormitoryName: useRef<HTMLDivElement>(null),
  ...
  useEffect(() => {
    ...
    if (!formData.dormitoryName.trim()) {
      if (isSubmitted) newErrors.dormitoryName = "생활관을 선택하세요.";
    }

    const roomStr = formData.roomId.trim();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(errors).length > 0 || !isEmailVerified) {
      if (!isEmailVerified) {
        setErrors((prev) => ({
          ...prev,
          verificationCode: "이메일 인증이 필요합니다.",
        }));
      }
      return;
    }
    ...
    <div className="flex flex-col" ref={formRefs[field as keyof typeof formRefs]}>
      ...
      <div className="flex gap-2">
        <div className="flex-1 relative group">
          <div
            className={`absolute left-4 top-1/2 -translate-y-1/2 z-10 transition-colors duration-200 ${focusedField === field ? "text-[#5eb9ca]" : "text-[#adc0c2]"
              }`}
          >
          ...
          <input
            ...
            className={`w-full ${isEmailVerified && field === "verificationCode"
                ? "bg-slate-100/50"
                : "bg-white"
          ...
          {field === "verificationCode" &&
            ...
            !isEmailVerified &&
            timeLeft > 0 && (
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[12px] text-red-500 font-bold">
                {formatTime(timeLeft)}
              </span>
            )}
        </div>
      ...
      <div className="h-[18px] mt-0.5 ml-1">
        {errors[field] && (
          <p className="text-[10px] text-red-500 font-bold">* {errors[field]}</p>
        )}

        {successMsgs[field] && !errors[field] && (
          <p className="text-[10px] text-green-500 font-bold flex items-center gap-1">
            <CheckCircle size={10} /> {successMsgs[field]}
          </p>
        )}
      </div>
    </div>
  ...
  return (
    <div className="bg-[#f0f9ff] min-h-screen w-full max-w-[448px] mx-auto relative antialiased flex flex-col shadow-2xl overflow-hidden">
      ...
      {alertConfig.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-8 bg-[#054a57]/30 backdrop-blur-[4px]">
          <div className="bg-white w-full max-w-[320px] rounded-[32px] shadow-2xl p-8 text-center animate-in zoom-in duration-300">
            ...
            <p className="text-[14px] font-medium text-[#7aaeb7] mb-8 leading-relaxed">
              {alertConfig.message}
            </p>
            ...
            <button
              ...
              className={`w-full h-[56px] text-white font-bold rounded-[20px] active:scale-[0.96] shadow-lg transition-transform ${alertConfig.type === "success"
                  ? "bg-[#5eb9ca] shadow-[#5eb9ca]/20"
                  : "bg-[#adc0c2] shadow-gray-200"
                }`}
            >
      ...
      <div className="pt-16 px-8 pb-6 flex items-center gap-4 shrink-0">
        ...
        <div>
          <h1 className="font-bold text-[26px] text-[#054a57] tracking-tight">
            회원가입
          </h1>
      ...
      <div className="flex-1 px-6 pb-12 overflow-y-auto custom-scrollbar">
        <div className="bg-white/70 backdrop-blur-lg rounded-[32px] p-7 shadow-xl shadow-blue-900/5 border border-white">
          <form onSubmit={handleSubmit} className="space-y-1">
            ...
            <div className="flex flex-col" ref={formRefs.dormitoryName}>
              <label className="text-[10px] font-bold text-[#829496] ml-1 mb-1 uppercase tracking-wider">
                생활관 동수
              </label>
              ...
              <div className="relative">
                <button
                  ...
                  className={`w-full bg-white border-2 ${openSelect ? "border-[#5eb9ca]" : "border-white"
                    } rounded-[18px] px-4 h-[54px] flex items-center justify-between transition-all shadow-sm`}
                >
                  <div className="flex items-center gap-3">
                    <Building2
                      className={`size-[18px] ${openSelect ? "text-[#5eb9ca]" : "text-[#adc0c2]"
                        }`}
                    />
                    <span className="text-[14px] font-bold text-[#054a57]">
                      {formData.dormitoryName}
                    </span>
                  </div>

                  <ChevronDown
                    className={`size-4 text-[#94a3b8] transition-transform ${openSelect ? "rotate-180" : ""
                      }`}
                  />
                </button>
                ...
                {openSelect && (
                  <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-[22px] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                    {dormitoryOptions.map((opt) => (
                      ...
                      >
                        {opt.name}
                        {formData.dormitoryName === opt.name && (
            ...
            <div className="flex flex-col" ref={formRefs.roomId}>
              ...
              <div className="relative group">
                <Home
                  className={`absolute left-4 top-1/2 -translate-y-1/2 size-[18px] transition-colors duration-200 ${focusedField === "roomId" ? "text-[#5eb9ca]" : "text-[#adc0c2]"
                    }`}
                />
                ...
                <input
                  ...
                  className={`w-full bg-white border-2 ${errors.roomId ? "border-red-400" : "border-white"
                    } rounded-[18px] pl-11 pr-4 h-[54px] text-[14px] font-bold text-[#054a57] focus:outline-none focus:border-[#5eb9ca] transition-all shadow-sm`}
                />
              </div>
              ...
              <div className="h-[20px] mt-0.5 ml-1">
                {errors.roomId && (
                  <p className="text-[10px] text-red-500 font-bold">* {errors.roomId}</p>
                )}
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full h-[60px] bg-[#5eb9ca] rounded-[22px] text-white font-bold text-[17px] shadow-xl shadow-[#5eb9ca]/30 mt-6 active:scale-95 transition-all disabled:bg-slate-300 flex items-center justify-center"
            >
              {isLoading ? "준비 중..." : "가입하기"}
            </button>
          </form>
